...
class TestParseBs:
    def test_ch4_species(self, ch4_bs_path):
        species, frame, styles, specs, _ = parse_bs(ch4_bs_path)
    ...
    def test_from_string(self):
        ...
        species, frame, styles, specs, _ = parse_bs(content)
        assert species == ["X"]
...
class TestParsePoly:
    def test_poly_default_alpha(self):
        content = "atom Ti 0 0 0\npoly Ti\n"
        _, _, _, _, poly_specs = parse_bs(content)
        assert len(poly_specs) == 1
        assert poly_specs[0].centre == "Ti"
        assert poly_specs[0].alpha == 0.4
        assert poly_specs[0].colour is None

    def test_poly_with_alpha(self):
        content = "atom Ti 0 0 0\npoly Ti 0.6\n"
        _, _, _, _, poly_specs = parse_bs(content)
        assert poly_specs[0].alpha == pytest.approx(0.6)
        assert poly_specs[0].colour is None

    def test_poly_with_rgb_colour(self):
        content = "atom Ti 0 0 0\npoly Ti 0.5 0.2 0.4 0.9\n"
        _, _, _, _, poly_specs = parse_bs(content)
        assert poly_specs[0].alpha == pytest.approx(0.5)
        assert poly_specs[0].colour == pytest.approx((0.2, 0.4, 0.9))

    def test_poly_with_grey_colour(self):
        content = "atom Ti 0 0 0\npoly Ti 0.3 0.7\n"
        _, _, _, _, poly_specs = parse_bs(content)
        assert poly_specs[0].alpha == pytest.approx(0.3)
        assert poly_specs[0].colour == pytest.approx(0.7)

    def test_no_poly_lines(self):
        content = "atom X 1.0 2.0 3.0\nspec X 0.5 red\n"
        _, _, _, _, poly_specs = parse_bs(content)
        assert poly_specs == []
